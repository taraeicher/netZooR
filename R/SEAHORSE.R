...
seahorseFormatForUI <- function(input_directory, result_directory, output_directory){
  ...
  # Loop through all files and obtain results to write.
  tissue_names <- list.files(result_directory)
  results <- lapply(1:length(tissue_names), function(i)){
    
    # Read the input and result files.
    filenameRes = paste0(result_directory, tissue_names[i])
    filenameIn = paste0(input_directory, tissue_names[i])
    
    pathwayEnrichment <- getPathwayEnrichment(filenameRes)
    dataDict <- makeDataDict(filenameIn)
    geneGene <- makeGeneGene(filenameRes)
    expression <- makeExpression(filenameIn)
    mapping <- makeMapping(filenameIn)
    phenotype <- makePhenotype(filenameIn)
    phenotypeGene <- makePhenotypeGene(filenameRes)
    phenotypePhenotype <- makePhenotypePhenotype(filenameRes)
    
  }
  
  # Write each file.
  writePathwayEnrichment(results = results, file = paste(output_directory, "all_gsea_results.tsv", sep = "/"))
  writeDataDictionary(results = results, file = paste(output_directory, "data_dictionary.tsv", sep = "/"))
  writeGeneGeneAssociations(results = results, file = paste(output_directory, "geneexpression2geneexpression.tsv", sep = "/"))
  writeExpression(results = results, file = paste(output_directory, "geneexpression_data.tsv", sep = "/"))
  writeGeneMapping(results = results, file = paste(output_directory, "human_ensembl2symbol_map.tsv", sep = "/"))
  writePhenotype(results = results, file = paste(output_directory, "metadata.tsv", sep = "/"))
  writePhenotypeGeneAssociations(results = results, file = paste(output_directory, "metadata2expression.tsv", sep = "/"))
  writePhenotypePhenotypeAssociations(results = results, file = paste(output_directory, "metadata2metadata.tsv", sep = "/"))
}
